const CRITERIA = [
  { key: "build", label: "Balance & build", max: 20, hint: "Even frame, stands over the ground, looks an athlete." },
  { key: "muscle", label: "Muscle & condition", max: 20, hint: "Hard and fit. Not fat, not tucked-up." },
  { key: "grooming", label: "Grooming", max: 15, hint: "Coat, mane and tail clean and shining." },
  { key: "legs", label: "Legs & feet", max: 10, hint: "Clean legs, tidy feet. No messy wraps." },
  { key: "presence", label: "Movement & demeanour", max: 20, hint: "Walks out freely. Alert, manners in the yard." },
...
const OFFICIAL_SCRATCH = { 1:[1,5,12], 2:[8], 4:[1,3,7], 6:[7,11], 7:[9], 8:[13,15,17], 10:[3,8,11,13,15,16,18] };
function applyOfficialScratchings(card) {
  (card.races || []).forEach(r => {
    const ids = OFFICIAL_SCRATCH[r.raceNumber] || [];
    (r.horses || []).forEach(h => { if (ids.includes(Number(h.cloth))) h.scratched = true; });
    r.scratched = (r.horses || []).filter(h => h.scratched).length;
    r.live = (r.horses || []).filter(h => !h.scratched).length;
  });
}
async function boot() {
  if (CARD) return CARD;
...
async function renderJudgeHome() {
  ...
  view.innerHTML = `<section class="hero"><div class="kicker">Caulfield · 19 September 2026 · Skyline Suites</div><h1>Best presented in the yard</h1><p>All ten tables. Tap This is us on your race. Scores stay on this phone.</p></section><div class="card"><h2>Allocated tables</h2><div id="races"></div></div>`;
  $("#races").innerHTML = state.board.map(r => {
    const att = r.attachment;
    const g1 = groupTag(r) === "GROUP 1" ? " g1" : "";
    return `<div class="race"><div class="rn${g1}">${r.raceNumber}</div><div class="meta"><div class="kicker">${groupTag(r)} · out ${r.leaveSuite || ""}</div><b>${r.name}</b><div class="who">${(att?.members || []).join(" · ")}</div></div><button class="btn soil" data-claim="${att?.partyId || ""}">This is us</button></div>`;
  }).join("");
...
function paintHorse() {
  ...
  view.innerHTML = `<section class="hero"><div class="kicker">${groupTag(race)} · ${fmtTime(race.time)} · ${race.distance} · ${live.length} to judge</div><h1>R${race.raceNumber} ${race.name}</h1><p>Present as they walk to the start. ${(race.attachment?.members || []).join(" · ")}</p></section><div class="yard"><div class="progress">${live.map((x,i) => `<button class="${i===horseIndex?"on":""} ${totalOf(x.marks)? "done":""}" data-i="${i}">${x.cloth}</button>`).join("")}</div><div class="horse-pane" id="pane"></div>${scr.length ? `<div class="muted" style="padding:0 6px">Scratched · ${scr.map(x => x.cloth + " " + x.name).join(" · ")}</div>` : ""}<button class="btn ghost" style="width:100%" id="back">Back to tables</button></div>`;
  document.querySelectorAll(".progress [data-i]").forEach(b => b.onclick = () => { horseIndex = Number(b.dataset.i); paintHorse(); });
  $("#back").onclick = () => { try { localStorage.removeItem(LS); } catch {} location.hash = "#/judge"; };
  mountPane(h);
  mountDock();
}
...
function mountDock() {
  ...
  dock.innerHTML = `<button class="btn ghost" id="prev" style="color:#f4ead3;border-color:#6d5c4a">Prev</button><div class="lead">${lead ? lead.cloth + " " + lead.name + " · " + totalOf(lead.marks) : ranked.length + " / " + live.length + " scored"}</div><button class="btn gold" id="save" style="margin:0;width:auto">${last ? "Save" : "Next"}</button>`;
...
async function renderBoard() {
  ...
  $("#dock")?.remove();
  const state = await api("/api/state");
  view.innerHTML = `<section class="hero"><div class="kicker">Skyline Suites · live</div><h1>Best presented</h1><p>${state.meetingName}. Winners lock as each table finishes in the yard.</p></section><div class="board-grid">${state.board.map(r => { const w = r.winner; return `<div class="brow ${w ? "hot" : ""}"><div class="r">${r.raceNumber}</div><div><b>${r.name}</b><div class="muted">${w ? w.cloth + " " + w.name : "Still in the yard"} · ${fmtTime(r.time)}</div></div><div style="font-family:'Instrument Serif',Georgia,serif;font-size:30px;text-align:right">${w ? w.total : "—"}</div></div>`; }).join("")}</div>`;
}
...
function bindNav() {
  document.querySelectorAll("[data-nav]").forEach(a => {
    const go = (e) => { e.preventDefault(); e.stopPropagation(); goTab(a.dataset.nav); };
    a.onclick = go;
...
function start() {
  try {
    ...
    if (!(window.CARD_DATA && window.CARD_DATA.races) && !((window.CARD_RACES_A||[]).length + (window.CARD_RACES_B||[]).length)) {
      view.innerHTML = '<section class="hero"><h1>Card did not load</h1><p>Refresh the page on this phone.</p></section>';
      return;
    }
    bindNav();
    ...
    route();
    setInterval(() => { if (/board|host/.test(location.hash)) route(); }, 8000);
  } catch (e) {
